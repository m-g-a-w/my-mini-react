...
export const beginWork = (wip: FiberNode, renderLane: Lane) => {
	...
	if (__DEV__) {
		console.log('beginWork:', {
			tag: wip.tag,
			tagName: getTagName(wip.tag),
			type: wip.type,
			typeofType: typeof wip.type,
			isFunction: typeof wip.type === 'function'
		});
	}
...
function updateSuspenseComponent(workInProgress: FiberNode) {
	...
	if (didSuspend) {
		...
		// 检查是否应该从 fallback 状态切换回正常状态
		// 如果当前显示的是 fallback，但这次没有 suspend，则切换到正常状态
		const currentChild = current.child as FiberNode;
		if (currentChild && currentChild.sibling !== null) {
			// 当前有 sibling，说明之前显示的是 fallback
			// 这次没有 suspend，应该切换到正常状态
			showFallback = false;
		}
	}
